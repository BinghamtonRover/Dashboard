...
class VideoModel extends Model {
  ...
  Map<CameraName, int> framesThisSecond = {
    for (final name in CameraName.values) name: 0,
  };
  ...
  void handleData(VideoData newData) {
    if ((newData.hasFrame() &&
            newData.details.name == CameraName.CAMERA_NAME_UNDEFINED) ||
    ...
    if (newData.hasFrame() &&
        newData.details.status == CameraStatus.CAMERA_ENABLED) {
  ...
  Future<void> toggleCamera(CameraName name, {required bool enable}) async {
    ...
    _handshake = null;
    details.status =
        enable ? CameraStatus.CAMERA_ENABLED : CameraStatus.CAMERA_DISABLED;
