...
/// The settings page, shown as a dialog with tabs for each section.
class SettingsPage extends ReactiveWidget<SettingsBuilder> {
  ...
  Widget _network(BuildContext context, SettingsBuilder model) => ListView(
    ...
    children: [
      ValueEditor<NetworkSettings>(
        ...
        children: [
          SocketEditor(
            name: "Subsystems socket",
            model: model.network.dataSocket,
          ),
          SocketEditor(name: "Video socket", model: model.network.videoSocket),
          SocketEditor(
            name: "Autonomy socket",
            model: model.network.autonomySocket,
          ),
          SocketEditor(
            name: "Base station socket",
            model: model.network.baseSocket,
          ),
          SocketEditor(
            name: "Tank IP address",
          ...
          if (Platform.isWindows)
            ListTile(
              ...
              subtitle: const Text(
                "You may need to change these if the rover will not connect",
              ),
  ...
  Widget _baseStation(SettingsBuilder model) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
  ...
  Widget _arm(SettingsBuilder model) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
      ValueEditor<ArmSettings>(
        ...
        children: [
          ...
          NumberEditor(name: "Wrist roll increment", model: model.arm.wristRoll),
  ...
  Widget _science(SettingsBuilder model) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
      ValueEditor<ScienceSettings>(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text(
              "Graphs can either be forced to fit the page or allowed to scroll\nMight be inconvenient for desktop users",
            ),
  ...
  Widget _dashboard(BuildContext context, SettingsBuilder model) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
      ValueEditor<DashboardSettings>(
        ...
        children: [
          NumberEditor(
            ...
            subtitle:
                "This does not affect the rover's cameras. Useful for limiting the CPU of the dashboard",
          ...
          SwitchListTile(
            ...
            subtitle: const Text(
              "If enabled, cameras can only be controlled by a separate operator",
            ),
          ...
          SwitchListTile(
            ...
            subtitle: const Text(
              "Default to tank controls instead of modern drive controls",
            ),
          ...
          NumberEditor(
            ...
            subtitle:
                "The maximum acceleration of the drive (joystick input per second)",
            model: model.dashboard.driveRateLimit,
          ),
          NumberEditor(
            name: "Throttle Rate Limit",
            subtitle:
                "The maximum acceleration of the drive throttle (input per second)",
          ...
          Row(
            children: [
              const SizedBox(
                width: 200,
                child: ListTile(title: Text("Theme mode")),
              ),
              const Spacer(),
  ...
  Widget _easterEggs(SettingsBuilder model) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
  ...
  Widget _misc(BuildContext context) => ListView(
    padding: const EdgeInsets.all(12),
    children: [
      ...
      ListTile(
        ...
        onTap: () =>
            showDialog<void>(context: context, builder: (_) => TimerEditor()),
      ),
      ListTile(
          ...
          children: [
            ...
            Center(
              child: TextButton(
                onPressed: () => launchUrl(
                  Uri.parse(
                    "https://github.com/BinghamtonRover/Dashboard/graphs/contributors",
                  ),
                ),
