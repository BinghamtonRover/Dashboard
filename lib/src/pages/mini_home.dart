    ...
    child: Row(
      ...
      children: [
        SizedBox(
          ...
          child: FittedBox(
            fit: BoxFit.fill,
            child: Icon(getBatteryIcon(model.batteryPercentage)),
          ),
        ),
        const SizedBox(width: 5),
        Column(
...
class MiniHomeSystemStatus extends ReusableReactiveWidget<LogsViewModel> {
  ...
  Widget statusIcon(Device? device) {
    final socket = models.sockets.socketForDevice(
    ...
    Color? iconColor = switch (lowestLevel) {
      ...
      BurtLogLevel.info ||
      BurtLogLevel.debug ||
      BurtLogLevel.trace => Colors.green,
      BurtLogLevel.warning =>
        Colors
            .yellow, // Separate line in case if we need to change it at any point
      ...
      Expanded(
        ...
        child: Padding(
          ...
          child: GestureDetector(
            onDoubleTap: () {
              final wasEnabled = models.sockets.isEnabled;
              models.sockets.disable();
              Navigator.of(context)
                ..pop()
                ..pushNamed(Routes.screenSaver).then((_) {
                  if (wasEnabled) {
                    models.sockets.enable();
                  }
                });
            },
            child: Image.asset(
