...
class PositionMetrics extends Metrics<RoverPosition> {
  ...
  String getRTKString(RTKMode mode) => switch(mode) {
    ...
    MetricLine("  RTK Mode: ${getRTKString(data.gps.rtkMode)}"),
		...
		MetricLine("  Pitch: ${data.orientation.pitch.toStringAsFixed(2)}°", severity: getRotationSeverity(data.orientation.pitch)),
		MetricLine("  Roll: ${data.orientation.roll.toStringAsFixed(2)}°", severity: getRotationSeverity(data.orientation.roll)),
		MetricLine("  Yaw: ${data.orientation.yaw.toStringAsFixed(2)}°"),
